...
def process_day(day_file: str) -> pl.LazyFrame:
    return (
        ...
        .filter(
            ...
            | (
                pl.col("MAXDRAUGHT").arr.lengths() >= DRAUGHT_COUNT_THRESHOLD
            )  # draught changed
            ...
            )  # no rate of trun detected
            | (
                pl.col("DESTINATION").arr.lengths() >= DESTINATION_COUNT_THRESHOLD
            )  # destination changed
        )
        .with_columns(
            pl.when(pl.col("SOG") == 1)
            .then(True)
            .otherwise(False)
            .cast(pl.Boolean)
            .alias("no_sog_event"),
            ...
            pl.when(pl.col("DESTINATION").arr.lengths() >= 2)
            .then(True)
            .otherwise(False)
            .cast(pl.UInt8)
            .alias("destination_changed_event"),
            pl.when(pl.col("MAXDRAUGHT").arr.lengths() >= 2)
            .then(True)
            .otherwise(False)
            .cast(pl.UInt8)
