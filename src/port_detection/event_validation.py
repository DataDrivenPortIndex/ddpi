...
PORT_EVENTS_FILES = [
    "port_events2020.parquet",
    "port_events2021.parquet",
    "port_events2022.parquet",
]
MIN_DAYS = 30
...
EVENT_SCORE_COLUMNS = [
    ...
    "no_movement_event",
    "drifting_event",
    "draught_changed_event",
    "towing_event",
    "destination_changed_event",
    "moored_event",
    "anchored_event",
]
...
def combine_port_events(port_event_dfs: List[pl.DataFrame]):
    ...
    df.groupby("h3_cell").agg(
        ...
        (
            (pl.col("last_timestamp") - pl.col("first_timestamp")).cast(pl.Int64())
            / MICROSECONDS_IN_DAY
        ).alias("event_life_span")
    ).filter(
    ...
    ).with_columns(pl.col("number_of_unique_vessels").clip(1, 30)).write_csv(
        "validated_port_events.csv"
    )
