...
def get_point_of_wpi_port(wpi_gdf, world_gdf):
    wpi_gdf["nearest_point"] = wpi_gdf.apply(
        lambda x: nearest_points(world_gdf.geometry.boundary, x.geometry)[0], axis=1
    )

    wpi_gdf["distance"] = wpi_gdf.apply(
        lambda x: fast_geo_distance.geodesic(
            x.Latitude, x.Longitude, x.nearest_point.y, x.nearest_point.x
        ),
        axis=1,
    )
...
def main():
    ...
    ddpi_buffered_gdf["geometry"] = ddpi_buffered_gdf["geometry"].buffer(
        WPI_DISTANCE_THRESHOLD / 111111
    )
    ...
    wpi_no_in_ddpi_gdf = gpd.GeoDataFrame(
        wpi_no_in_ddpi_gdf, geometry=wpi_no_in_ddpi_gdf.nearest_point
    )

    wpi_no_in_ddpi_gdf["geometry"] = wpi_no_in_ddpi_gdf["geometry"].buffer(
        WPI_POINT_BUFFER / 111111
    )
    ...
    wpi_no_in_ddpi_gdf = gpd.sjoin(
        wpi_no_in_ddpi_gdf, countries_df, how="left", predicate="intersects"
    )

    wpi_no_in_ddpi_gdf = wpi_no_in_ddpi_gdf[
        ["id", "is_anchorage", "geometry", "country_code"]
    ]
